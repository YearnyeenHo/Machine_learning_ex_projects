...
for i = 1:m
  ...
  vecY(y(i)) = 1; 
  tmp = -vecY.*log(oa)-(1 - vecY).*log(1 - oa);
  
  delta_3 = oa - vecY;
  delta_2 = Theta2'*delta_3.*[0; sigmoidGradient(hidz)];
  
  
  Theta2_grad = Theta2_grad + delta_3*hida';
  Theta1_grad = Theta1_grad + delta_2(2:end)*smp;
...
J = J/m;
reg = Theta1(:, 2:end);
reg = sum(power(reg, 2), 2);
reg = sum(reg);

t2Sum = Theta2(:, 2:end);
t2Sum = sum(power(t2Sum, 2), 2);
t2Sum = sum(t2Sum);

reg = (lambda/(2*m))*(reg + t2Sum);
J = J + reg;
